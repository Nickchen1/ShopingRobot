...
const int slowSpeed(900);
const int turnSpeed (1400);
const int carSpeed = normalSpeed;

const unsigned long kOneStepDelay (500);
...
const int FRONT_WIRE_SENSOR_TRIGGER(2);
const int BACK_WIRE_SENSOR_TRIGGER(3);
const int LEFT_WIRE_SENSOR_TRIGGER(4);
const int RIGHT_WIRE_SENSOR_TRIGGER(5);
...
static int near_wire_sensor_state_real(B0000);
...
static char UpdateLineSensor() {
  near_wire_sensor_state_real = digitalRead(FRONT_WIRE_SENSOR_TRIGGER) << 3 |
                                digitalRead(BACK_WIRE_SENSOR_TRIGGER)  << 2 |
                                digitalRead(LEFT_WIRE_SENSOR_TRIGGER)  << 1 |
                                digitalRead(RIGHT_WIRE_SENSOR_TRIGGER);

  front_line_sensor_state_real = digitalRead(FRONT_WIRE_SENSOR_S1_PIN) << 4 |
                                 digitalRead(FRONT_WIRE_SENSOR_S2_PIN) << 3 |
                                 digitalRead(FRONT_WIRE_SENSOR_S3_PIN) << 2 |
                                 digitalRead(FRONT_WIRE_SENSOR_S4_PIN) << 1 |
                                 digitalRead(FRONT_WIRE_SENSOR_S5_PIN);

  back_line_sensor_state_real = digitalRead(BACK_WIRE_SENSOR_S1_PIN) << 4 |
                                digitalRead(BACK_WIRE_SENSOR_S2_PIN) << 3 |
                                digitalRead(BACK_WIRE_SENSOR_S3_PIN) << 2 |
                                digitalRead(BACK_WIRE_SENSOR_S4_PIN) << 1 |
                                digitalRead(BACK_WIRE_SENSOR_S5_PIN);

  left_line_sensor_state_real =  digitalRead(LEFT_WIRE_SENSOR_S1_PIN) << 4 |
                                 digitalRead(LEFT_WIRE_SENSOR_S2_PIN) << 3 |
                                 digitalRead(LEFT_WIRE_SENSOR_S3_PIN) << 2 |
                                 digitalRead(LEFT_WIRE_SENSOR_S4_PIN) << 1 |
                                 digitalRead(LEFT_WIRE_SENSOR_S5_PIN);

  right_line_sensor_state_real = digitalRead(RIGHT_WIRE_SENSOR_S1_PIN) << 4 |
                                 digitalRead(RIGHT_WIRE_SENSOR_S2_PIN) << 3 |
                                 digitalRead(RIGHT_WIRE_SENSOR_S3_PIN) << 2 |
                                 digitalRead(RIGHT_WIRE_SENSOR_S4_PIN) << 1 |
                                 digitalRead(RIGHT_WIRE_SENSOR_S5_PIN);

}

LineSensorState near_sensor(int near_sensor_state_real) {
  switch (near_sensor_state_real) {
    case B1111:
      return kWireSensorStateGo;
      break;

    case B0111:
      return kWireSensorStateFrontStop;
      break;

    case B1011:
      return kWireSensorStateBackStop;
      break;

    case B1101:
      return kWireSensorStateLeftStop;
      break;

    case B0000:
      return kWireSensorStateRightStop;
      break;
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (count_count_ - start_time > kOneStepDelay ) {
        if ((position_sensor(left_line_sensor_state_real) == kLineSensorStateMid || position_sensor(right_line_sensor_state_real) == kLineSensorStateMid ) && flag == true) {
          //StopMove();
          whiteline++;
          flag = false;
# ifdef _DEBUG_PRINT_STATUS_
        ...
        else if (near_sensor(near_wire_sensor_state_real) == kWireSensorStateFrontStop && flag == true) {
          //StopMove();
          whiteline++;
          flag = false;
...
void ForwardSlowStep(int count) {
  bool flag = false;
  int whiteline = 0;
  for (;;) {
    flag = true;
...
# ifdef _DEBUG_PRINT_STATUS_
        ...
        else if (near_sensor(near_wire_sensor_state_real) == kWireSensorStateBackStop && flag == true) {
          //StopMove();
          whiteline++;
          flag = false;
...
void BackwardSlowStep(int count) {
  bool flag = false;
  int whiteline = 0;
  for (;;) {
    unsigned long start_time = count_count_;
...
# ifdef _DEBUG_PRINT_STATUS_
        ...
        else if (near_sensor(near_wire_sensor_state_real) == kWireSensorStateLeftStop && flag == true) {
          //StopMove();
          whiteline++;
          flag = false;
...
# ifdef _DEBUG_PRINT_STATUS_
        ...
        else if (near_sensor(near_wire_sensor_state_real) == kWireSensorStateRightStop && flag == true) {
          //StopMove();
          whiteline++;
          flag = false;
...
void setup() {
  ...
  pinMode(FRONT_WIRE_SENSOR_TRIGGER, INPUT);
  pinMode(BACK_WIRE_SENSOR_TRIGGER, INPUT);
  pinMode(LEFT_WIRE_SENSOR_TRIGGER, INPUT);
  pinMode(RIGHT_WIRE_SENSOR_TRIGGER, INPUT);
...
#ifdef _DEBUG_PRINT_SENSOR_
  UpdateLineSensor();
  Serial.print(millis()); Serial.print("\t");
  Serial.print("STATUS:"); Serial.print("\t");
  Serial.print(front_line_sensor_state_real, BIN); Serial.print("\t");
  Serial.print(back_line_sensor_state_real, BIN); Serial.print("\t");
  Serial.print(left_line_sensor_state_real, BIN); Serial.print("\t");
  Serial.print(right_line_sensor_state_real, BIN); Serial.print("\t");
  Serial.println(near_wire_sensor_state_real, BIN);
#endif // _DEBUG_PRINT_SENSOR_
  ...
  if (Serial.available() > 0) {
    ...
    switch (action) {
      case'F':
        ...
        break;
      case'f':
        ForwardSlowStep(count);
        break;
      case'B':
        BackwardStep(count);
        break;
      case'b':
        BackwardSlowStep(count);
        break;
      case'L':
        LeftTurnStep(count);
        break;
